<x-layouts.app :title="__('Agregar Atributo a Item')">
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Nuevo Atributo de Item') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Aquí podrás crear un nuevo Atributo de item') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <div class="max-w-lg mx-auto mt-8 bg-white dark:bg-zinc-700 shadow-lg rounded-2xl p-6">
        <p class="text-lg font-semibold bg-white mb-4 dark:bg-zinc-700">Nuevo Atributo</p>

        <form 
        action="{{ route('tool_attributes.store') }}" 
        method="post"
        class="space-y-4">
        ...
        @csrf
            ...
            <flux:select name="toolType_id" id="toolType_id">
            ...
            <flux:select id="data_type" name="data_type" placeholder="Tipo de dato...">             
